...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    elif num_classes == 1 or dataset == 'TissueNet':
      self.names = TissueNet_class_name
  ...
  def show_img(self, pause = False, imgId = 'default'):
    ...
    img = self.imgs[imgId]
    img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  # 将BGR格式的图片转换为RGB格式
    plt.imshow(img_rgb)
    if pause:
      cv2.waitKey()
  ...
  def show_all_imgs(self, pause=False, time=0):
    ...
    #     sys.exit(0)
    for i, v in self.imgs.items():
        img_rgb = cv2.cvtColor(v, cv2.COLOR_BGR2RGB)  # 将BGR格式的图片转换为RGB格式
        plt.figure()  # 创建新的图形窗口
        plt.imshow(img_rgb)  # 使用matplotlib的imshow()函数显示图片
        plt.title('{}'.format(i))  # 设置图片标题
        plt.axis('off')
    else:
      self.ax = None
      nImgs = len(self.imgs)
